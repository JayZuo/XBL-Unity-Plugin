...
public class XboxLiveConfigurationEditor : EditorWindow
{
    ...
    private void OnGUI()
    {
        ...
        {
            EditorGUILayout.HelpBox("This plugin only supports the Xbox Live Creators Program and ID@Xbox program configured on UDC(Windows Dev Center).\n\nFor developers using XDP(Xbox Developer Portal), instead follow the docs at http://aka.ms/xbldocs", MessageType.Warning);
  ...
  ""Sandbox"": """",
        ...
        if (this.configuration != null)
        {
            ...
            PlayerSettings.productName = this.configuration.DisplayName;
            PlayerSettings.companyName = this.configuration.PublisherDisplayName;
